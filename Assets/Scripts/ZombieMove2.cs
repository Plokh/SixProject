...
public class ZombieMove2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ladoCima = true;//lado ao qual o personagem irá começar a andar
    ...
    {
        MudarEstado();
        playerDistancia = transform.position.x - player.transform.position.x;// Pego o resultado da distancia em que o player está e subtraio com  a distancia em que o personagem deve começar a atacar
        if (Mathf.Abs(playerDistancia) < ataqueDistancia)//caso o player esteja perto o suficiente o personagem começa a atacar e sai do modo espera
        {
            atacar = true;
    ...
    void Parado()//Faço a checagem do tempo em que o persongaem ira parar para recomeçar a patrulhar
    {
        tempoParado += Time.deltaTime;
        if (tempoParado <= duracaoParado)
        {
            animator.SetBool("Subindo", estaPatrulhando);//ativo a animação andando quando o mesmo atinge o tempo limite de ficar parado
            tempoPatrulha = 0;
        }
        else
        {
    ...
    {//Faço a checagem do tempo em que o persongaem ira patrulhar para depois fazer um intervalo
        ...
        {
            animator.SetBool("Subindo", estaPatrulhando);//Desativo a animação andando quando o mesmo atinge o tempo limite de ficar patrulhando
            Mover();
            tempoParado = 0;
        }
        else
    ...
    void Atirar()
    {
        if (playerDistancia < 0 && !ladoCima || playerDistancia > 0 && ladoCima)//identifico qual o lado em que o player esta para mudar a direção em que o personagem ira atirar
        {
            MudarDirecao();
        }
        if (atacar == true)
